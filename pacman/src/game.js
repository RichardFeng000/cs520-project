...
var currentRlModelKey;
var rlModel;

var getAvailableRlModelKeys = function() {
    var keys = [];
    var key;
    for (key in rlModelLibrary) {
        if (rlModelLibrary.hasOwnProperty(key) && rlModelLibrary[key] && rlModelLibrary[key].q_table) {
            keys.push(key);
        }
    }
    keys.sort();
...
var getRlModelByKey = function(key) {
    return rlModelLibrary[key];
};

var setCurrentRlModel = function(key) {
    var model = getRlModelByKey(key);
    if (!model || !model.q_table) {
        return false;
    }
    currentRlModelKey = key;
    rlModel = model;
    return true;
};

var cycleRlModel = function() {
    var keys = getAvailableRlModelKeys();
    var i;
    if (!keys.length) {
        currentRlModelKey = undefined;
        rlModel = undefined;
        return;
    }
    for (i=0; i<keys.length; i++) {
        if (keys[i] == currentRlModelKey) {
            setCurrentRlModel(keys[(i+1)%keys.length]);
            return;
        }
    }
    setCurrentRlModel(keys[0]);
};

var getCurrentRlModelLabel = function() {
    return currentRlModelKey || "NONE";
};
...
(function() {
    ...
    if (keys.indexOf("MODEL3") >= 0) {
        setCurrentRlModel("MODEL3");
        return;
    }
    setCurrentRlModel(keys[0]);
})();

var hasExternalRlModel = function() {
    return !!(rlModel && rlModel.q_table);
};

var getExternalRlModelValue = function(stateKey, actionLabel) {
    if (!hasExternalRlModel()) {
        return undefined;
    }
    var values = rlModel.q_table[stateKey];
    if (!values) {
        return undefined;
    }
    return values[actionLabel];
};

var getAiStrategyLabel = function(strategy) {
...
(function(){
    ...
    var i, invincible, ai, aiMode, isDrawPath, isDrawTarget;
    isDrawPath = {};
    isDrawTarget = {};
    backupCheats = function() {
        invincible = pacman.invincible;
        ai = pacman.ai;
        aiMode = pacman.aiMode;
        for (i=0; i<5; i++) {
